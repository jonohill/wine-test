...
/* ARM64 system call interface for macOS */
#define SYSCALL_FUNC( name, nr ) \
    __ASM_GLOBAL_FUNC( name, \
                       "stp x16, x17, [sp, #-16]!\n\t" \
                       "mov x16, #(" #nr "|0x2000000)\n\t" \
                       "svc #0x80\n\t" \
                       "ldp x16, x17, [sp], #16\n\t" \
                       "cmn x0, #1, lsl #12\n\t" \
                       "b.hi 1f\n\t" \
...
__ASM_GLOBAL_FUNC( start,
                   ...
                   /* call wld_start(stack, &is_unix_thread) */
                   "\tadd x0, x29, #16\n\t"           /* x0 = stack pointer (skip saved fp/lr) */
                   "\tmov x1, sp\n\t"                 /* x1 = &is_unix_thread */
                   "\tstr xzr, [x1]\n\t"              /* is_unix_thread = 0 */
                   "\tbl _wld_start\n\t"
                   
                   /* check is_unix_thread flag */
                   "\tldr w1, [sp]\n\t"
                   "\tcbnz w1, 2f\n\t"
